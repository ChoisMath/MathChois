...
export async function sendVerificationEmail(
  ...
  await transporter.sendMail({
    from,
    to,
    subject: '[ChoisClass] 가입확인',
    html: `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 480px; margin: 0 auto; padding: 24px;">
        <h2 style="color: #1e40af; margin-bottom: 16px;">가입확인</h2>
        <p style="color: #374151; line-height: 1.6;">
          안녕하세요, <strong>${userName}</strong>님.
        </p>
        <p style="color: #374151; line-height: 1.6;">
          ChoisClass 회원가입을 완료하려면 아래 버튼을 클릭해 주세요.
        </p>
        <div style="text-align: center; margin: 32px 0;">
          <a href="${verifyUrl}"
             style="display: inline-block; padding: 12px 32px; background-color: #16a34a; color: #ffffff; text-decoration: none; border-radius: 8px; font-weight: 600; font-size: 16px;">
            가입확인
          </a>
        </div>
        <p style="color: #6b7280; font-size: 14px; line-height: 1.6;">
          버튼을 클릭하면 학생 계정이 생성되고 자동으로 로그인됩니다.
        </p>
        <p style="color: #9ca3af; font-size: 12px; margin-top: 24px;">
          본인이 요청하지 않았다면 이 이메일을 무시하세요. 이 링크는 24시간 동안 유효합니다.
        </p>
